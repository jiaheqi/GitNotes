...
4. 页面刷新 跳转到404界面，回到首页点击也没有反应：<https://nut.youle.game/#/error/404>
5. 主播认证 隐藏掉搜索公会的选项，流水区间需要改成只有一个截止时间
...
7. 主播申请记录 没有展示所属公会，查询条件只有申请状态，缺少申请玩法、申请时间（区间）
...
1. iOS 获取设备语言，根据语言实现全球化
2. iOS 支付和订阅去除分转元操作
3. iOS+Android 账号体系的三个变动
4. bugfix iOS 短信绑定和邮箱绑定的dialog关闭和返回会崩溃
5. iOS+Android 数数打点增加zeus_device_id字段

# 数数查询

## 查询测试的demo

使用bundle id 查询：where "#bundle_id" = 'com.topjoy.zeusdemo'
